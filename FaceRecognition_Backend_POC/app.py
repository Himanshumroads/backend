...
def train():
    ...
    if 'file' not in request.files:
        ...
        if file.mimetype not in app.config['file_allowed']:
            ...
            user_id = app.db.insert('INSERT INTO users(name, created, Age, Department, Designation) values(?,?,?,?,?)', [name, created, age, department, designation])

            if user_id:

                print("User saved in data", name, user_id, age, department, designation)
...
def recognize():
    if 'file' not in request.files:
        ...
        if file.mimetype not in app.config['file_allowed']:
            ...
            if user_id:
                user = get_user_by_id(user_id)
                message = {"message": "Hello {0} ".format(user["name"]),
                           "user": user}
                return success_handle(json.dumps(message))
            else:

                return error_handle("Sorry we can not found any people matched with your face image, Please Register yourself on our portal")
